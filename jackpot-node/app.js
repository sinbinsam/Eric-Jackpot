                ...
                request.query(jackpotQuery, (err, result) => {
                    if (err) {
                        ...
                        if (result.recordset.length > 0) {
                            ...
                            jackpotConditional(parseInt(result.recordsets[0].WinInDollars), result.recordsets[0].gamename)
                            pool.close()
                            return
...
function jackpotConditional(num, gameName) {
    if (num < 1000) {
        console.log('jackpot is less than 1000')
    } else if (num >= 1000) {
        console.log('jackpot is greater than or equal to 1000')
    }
}

function sjackpotConditional(num, gameName) {
    if (!num || num < 2500) {
...
function sendPlayCommand(track) {
    let date = new Date()
    axios({
        method: 'get',
        url: 'http://10.160.27.80:9000/jackpot/play?fileName=' + track + '&chickenDinner' + '&gameName' + gameName,
      })
      .then((res) => {
          if (res.status === 200) {
              return true
